...
def handle_query(user_query: str, wardrobe_choice: str) -> tuple[str, str, str]:
    ...
    if not user_query or not user_query.strip():
        return "⚠️ Please enter a search query so I know what to look for!", "", ""
    ...
    # Step 5: Format session["selected_item"] into a readable string
    item = session.get("selected_item", {})
    
    # We use markdown formatting here because Gradio Textboxes/Markdown panels render it beautifully
    listing_text = (
        ...
        f"_{item.get('description', 'No description available.')}_"
    )
